...
// Strip the "/km" or "/mi" unit suffix off a stored pace string for
// display. Coaches enter pace inconsistently — some include "/km", some
// don't — and the unit is implied app-wide. Use this anywhere a pace
// is rendered as a standalone value (the "PACE" stat box already has
// its own unit-free formatting expectation; the headline "at X" line
// looks weird with "/km" hanging off the end).
export function displayPace(paceStr) {
  if (!paceStr) return paceStr;
  return String(paceStr).replace(/\s*\/(km|mi)\b/gi, "").trim();
}
