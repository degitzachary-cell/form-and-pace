...
import { parseLocalDate, RUN_TYPES, isRunType } from "./helpers.js";
...
// Render a stored pace string with a consistent unit. Coaches enter
// pace inconsistently — some include "/km", some don't — so we strip
// whatever's there and re-append a single canonical suffix. Idempotent.
//
// Pace strings can also be free-form labels (e.g. "Easy 5:45 / HM
// 5:20–5:25"), in which case appending a unit would be wrong, so we
// only append when the string still parses as a single time / range
// of times after the strip.
//
// `unit` defaults to "km". When "mi", times are converted (slower
// numerically since a mile is longer than a km — pace × 1.609344).
export function displayPace(paceStr, unit = "km") {
  if (!paceStr) return paceStr;
  const stripped = String(paceStr).replace(/\s*\/(km|mi)\b/gi, "").trim();
  if (!stripped) return stripped;
  // Single time or hyphenated range. Honour both - and – (en-dash).
  const m = stripped.match(/^(\d{1,2}):(\d{2})(?:[-–](\d{1,2}):(\d{2}))?$/);
  if (!m) return stripped;
  const suffix = unit === "mi" ? "/mi" : "/km";
  if (unit === "km") return `${stripped}${suffix}`;
  // Convert each time from min/km → min/mi.
  const toMi = (mm, ss) => {
    const totalSec = (Number(mm) * 60 + Number(ss)) * 1.609344;
    const m2 = Math.floor(totalSec / 60);
    const s2 = Math.round(totalSec % 60);
    // Carry-over when seconds rounded up to 60.
    if (s2 === 60) return `${m2 + 1}:00`;
    return `${m2}:${String(s2).padStart(2, "0")}`;
  };
  const lo = toMi(m[1], m[2]);
  if (m[3] == null) return `${lo}${suffix}`;
  const hi = toMi(m[3], m[4]);
  return `${lo}-${hi}${suffix}`;
}

// Distance display in the athlete's preferred unit. Stored as km
// internally (rTSS / Strava / threshold pace are all km-based);
// converted to miles for display when the athlete has chosen mi.
export function displayDistance(km, unit = "km") {
...
// Average secs-per-km from a pace string that may be a single value
// ("5:30") or a hyphenated range ("5:30-5:45"). Used to predict run
// volume when the coach prescribed a time + pace but no distance.
// Returns null if the string doesn't parse.
function paceStrToAvgSecsPerKm(paceStr) {
  if (!paceStr) return null;
  const cleaned = String(paceStr).replace(/\s*\/(km|mi)\b/gi, "").trim();
  if (!cleaned) return null;
  const parts = cleaned.split(/[-–]/).map(p => paceStrToSecsPerKm(p.trim())).filter(p => p && p > 0);
  if (parts.length === 0) return null;
  if (parts.length === 1) return parts[0];
  return (parts[0] + parts[parts.length - 1]) / 2;
}

// Predict distance (km) from a duration (minutes) and a pace string
// (single or range). Returns null when either input is missing or the
// pace string is free-form text we can't compute against.
export function predictDistanceKm(durationMin, paceStr) {
  if (!durationMin || durationMin <= 0) return null;
  const secsPerKm = paceStrToAvgSecsPerKm(paceStr);
  if (!secsPerKm) return null;
  const km = (Number(durationMin) * 60) / secsPerKm;
  return Math.round(km * 10) / 10;
}

// Reformat a free-typed pace string ("MM:SS" or partial) into the
// canonical M:SS / MM:SS shape as the user types. Strips everything
// that isn't a digit and re-inserts the colon between minutes and
// seconds, so "435" → "4:35", "1235" → "12:35", and pasted junk like
// "4:35/km" → "4:35". Capped at 4 digits — nobody runs at threshold
// in tens-of-hours per km. Use as the onChange for the threshold-pace
// input so coaches can't break the parser by typing "4.35" or "4'35".
export function maskPaceInput(raw) {
...
export function secsPerKmToPaceStr(secsPerKm, withUnit = true) {
  ...
  // Round total seconds first so 359.7 renders as 6:00, not 5:60.
...
function estimateThresholdPaceSecsPerKm(pbs) {
...
// RPE → colour band. Blue→green→yellow→orange→red across the 1-10 Borg
// scale. Used wherever a single RPE number is rendered so the eye reads
// effort intensity without parsing the number. Returns a CSS colour
// (hex) suitable for `color:` or `background:`.
//
// Bin boundaries:
//   1–2  cool blue       (recovery)
//   3–4  green            (easy / aerobic)
//   5–6  amber/yellow     (steady)
//   7–8  orange           (tempo / threshold)
//   9–10 red              (VO2 / race)
//
// Pace ranges in `rpe_target` ("3-4") use the lower bound for binning
// so an "EASY 3-4" pill shows green, not amber.
export function rpeColor(rpe) {
  if (rpe == null || rpe === "") return null;
  let n;
  if (typeof rpe === "string") {
    const m = rpe.match(/(\d+(?:\.\d+)?)/);
    n = m ? parseFloat(m[1]) : NaN;
  } else {
    n = Number(rpe);
  }
  if (!Number.isFinite(n)) return null;
  if (n <= 2) return "#3F5A6B";  // cool (recovery)
  if (n <= 4) return "#5C7A4A";  // green (easy)
  if (n <= 6) return "#A0792F";  // amber/yellow (steady) — darker than #C8A249 for AA contrast on cream
  if (n <= 8) return "#C97A3A";  // orange (tempo)
  return "#B5523A";              // red (race / VO2)
}
...
function paceZones(thresholdSecsPerKm) {
...
function paceToZone(runSecsPerKm, thresholdSecsPerKm) {
...
function timeInZoneFromSplits(splits, thresholdSecsPerKm) {
...
function timeInZoneFromAverage({ durationSec, distanceKm, thresholdSecsPerKm }) {
  const acc = { Z1: 0, Z2: 0, Z3: 0, Z4: 0, Z5: 0, total: 0 };
...
export function gradeCompliance({ planned, actual }) {
  ...
  if (ratio <= 1.5) return "over";    // 20–50% above plan: did the work + more
  return "over";                       // >50% above plan: well past target
}
...
// Format a seconds duration for display. Runners read long reps in minutes,
// short recoveries in seconds: > 120 s renders as minutes ("6 min", or
// "6:30 min" when it isn't a clean minute); anything shorter stays "60 s".
const fmtSec = (sec) => {
  const s = Number(sec);
  if (!s || s <= 0) return "";
  if (s > 120) {
  ...
  switch (step.kind) {
    ...
    case "steady": {
      ...
      if (reps > 1) {
        ...
        const rTime = r.duration_s ? fmtSec(r.duration_s) : (r.duration_min ? `${r.duration_min} min` : null);
    ...
    case "interval": {
      ...
      const wDesc = w.distance_m ? `${w.distance_m} m` : (w.duration_s ? fmtSec(w.duration_s) : "?");
      const wPace = w.pace ? ` @ ${w.pace}` : "";
      const rDesc = r.distance_m ? `${r.distance_m} m` : (r.duration_s ? fmtSec(r.duration_s) : "");
...
export function aggregateSteps(steps) {
  ...
  // Predict km from a time-based segment when the step carries a
  // parseable pace string. Tempo + speed workouts are mostly defined
  // by duration ("5 min at T pace"), so without this every interval-
  // style workout used to roll up as 0 km.
  const predictKm = (durationMin, paceStr) => {
    if (!durationMin || durationMin <= 0 || !paceStr) return 0;
    const secs = paceStrToAvgSecsPerKm(paceStr);
    if (!secs) return 0;
    return (Number(durationMin) * 60) / secs;
  };
  const predictKmFromSec = (durationSec, paceStr) =>
    predictKm((Number(durationSec) || 0) / 60, paceStr);
  for (const s of steps) {
    ...
    if (s.kind === "warmup" || s.kind === "cooldown" || s.kind === "recovery") {
      ...
      if (unit === "min" && s.duration_min) {
        acc.duration_min += Number(s.duration_min) || 0;
        acc.distance_km  += predictKm(s.duration_min, s.pace);
      }
      ...
      if (unit === "min" && s.duration_min) {
        acc.duration_min += reps * (Number(s.duration_min) || 0);
        acc.distance_km  += reps * predictKm(s.duration_min, s.pace);
      }
      ...
      if (reps > 1) {
        ...
        // Predict km on rest segments when they're time-based + paced
        // (e.g. "60s jog @ E"). Falls through to 0 if no pace.
        if (r.duration_s)   acc.distance_km  += ((reps - 1) * predictKmFromSec(r.duration_s, r.pace));
        if (r.duration_min) acc.distance_km  += (reps - 1) * predictKm(r.duration_min, r.pace);
      }
    ...
    if (s.kind === "interval") {
      ...
      // Time-based work / recovery segments — predict km from pace so
      // tempo cruises (e.g. 4 × 5min @ T / 60s jog @ E) don't roll up
      // as 0 km. Pace falls back to the step's own pace for work, or
      // the recovery's pace for recovery; skip silently if neither has
      // a parseable absolute pace.
      if (w.duration_s && !w.distance_m) acc.distance_km += reps * predictKmFromSec(w.duration_s, w.pace || s.pace);
      if (r.duration_s && !r.distance_m) acc.distance_km += reps * predictKmFromSec(r.duration_s, r.pace);
    }
    if (s.kind === "strides") {
...
// Re-exported from lib/helpers under the legacy name. Treated as a run
// for null / unknown types so the bare "Run" default keeps working.
// Strength, Ride, Swim, Workout, Walk, Hike, etc. are NOT runs.
export const isRunActivityType = isRunType;
