...
export function formatStep(step) {
  ...
  switch (step.kind) {
    ...
    case "steady": {
      const dist = step.distance_km ? `${step.distance_km} km` : null;
      const time = fmtMin(step.duration_min);
      const lead = step.note?.trim() || "Steady";
      const bits = [dist || time, step.pace && `@ ${step.pace}`].filter(Boolean);
      return `${lead}${bits.length ? " · " + bits.join(" ") : ""}`;
    }
    case "interval": {
      // "20 × 400 m @ 3:35 w/ 60s rest"
      ...
      const recovery = rDesc ? ` w/ ${rDesc} rest` : "";
...
export function aggregateSteps(steps) {
  ...
  for (const s of steps) {
    if (s.duration_min) acc.duration_min += Number(s.duration_min) || 0;
    if (s.distance_km)  acc.distance_km  += Number(s.distance_km) || 0;
    if (s.kind === "interval") {
