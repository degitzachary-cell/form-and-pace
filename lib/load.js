...
export function formatStep(step) {
  ...
  switch (step.kind) {
    ...
    case "interval": {
      ...
      const rStyle = r.style === "float" ? " float" : rDesc ? " jog" : "";
...
// Treats null / undefined / "Run" / "Long Run" / "Easy Run" / "Tempo" /
// "Speed" / "Trail Run" / "Race" as runs. Strength, Ride, Swim, Workout,
// Walk, Hike etc. are NOT runs.
export function isRunActivityType(type) {
  if (type == null) return true; // legacy rows had no type — assume run
  const t = String(type).toLowerCase();
  return t === "run" || t === "long run" || t === "easy run" || t === "tempo"
    || t === "speed" || t === "trail run" || t === "race" || t === "recovery";
}
