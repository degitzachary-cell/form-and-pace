// ─── DATE HELPERS ─────────────────────────────────────────────────────────────
//
// Date string convention used throughout the app:
//   "YYYY-MM-DD" everywhere (activity_date, marker_date, session day-of-
//   plan, etc). Always means "this calendar day in the athlete's local
//   timezone" — never UTC. Training plans live on local-day boundaries,
//   so a Monday workout is a Monday wherever the athlete is sitting.
//
// parseLocalDate() is the canonical "YYYY-MM-DD → Date" parser. The
// trailing "T00:00:00" (no Z suffix) tells the Date constructor to
// interpret as local time. Without it, "2025-05-02" parses as UTC
// midnight, which on the western hemisphere shows up as the day before.
// Use this helper anywhere you'd otherwise type
...
// Snap any date string to its week's Monday. Returns a YYYY-MM-DD string.
...
export function thisMonday() {
  return snapToMonday(todayStr());
}

// ─── FORMAT HELPERS ───────────────────────────────────────────────────────────
export function fmtPace(mps, withUnit = true) {
  if (!mps || mps <= 0) return "–";
  // Round the TOTAL seconds first — rounding the remainder alone renders
  // 359.7s/km as "5:60/km" instead of "6:00/km".
...
// ─── KM AGGREGATION ───────────────────────────────────────────────────────────
// Run-only weekly distance — bike / strength / swim activities don't count
// toward running km. Pass-through legacy rows (no activity_type set) are
// assumed to be runs. Matches both the picker labels ("Easy Run", "Long
// Run") and the bare session-type strings ("EASY", "LONG RUN") since the
// session-log flow stores the latter.
// Canonical run-flavoured activity_type set. Shared by lib/load.js and
// lib/sessionMatching.js (each used to keep its own near-duplicate).
// Covers manual app types ("EASY", "LONG RUN", etc.) AND Strava sport
// types ("Run", "TrailRun", "VirtualRun"). Match by lowercase.
export const RUN_TYPES = new Set([
  "run", "easy", "easy run", "long", "long run", "tempo", "threshold", "speed", "parkrun",
  "recovery", "trail run", "trailrun", "race", "race day", "virtualrun",
]);
// True for any run-flavoured type. null/undefined counts as a run —
// legacy rows + the bare "Run" default.
export const isRunType = (t) => t == null || RUN_TYPES.has(String(t).toLowerCase());

// Sum km of RUN activities in a given ISO week (Mon–Sun). Compares
// YYYY-MM-DD strings directly so it's timezone-proof — `new Date(s)`
// parsed as UTC midnight could bucket a Sunday run into the prior week
// west of GMT.
export function weekKm(activities, email, weeksAgo = 0) {
  ...
  for (const a of activities) {
    ...
    if (!isRunType(a.activity_type)) continue;
    const d = a.activity_date;
    if (d && d >= lo && d <= hi) sum += parseFloat(a.distance_km || 0);
  }
  return sum;
}
