...
async function postFunction(path, payload) {
  try {
    ...
    // A 2xx with an unparseable/empty body is NOT a success — returning {}
    // here made gateway hiccups read as "done" to callers.
    return data ?? { error: "empty response from server", status: res.status };
  } catch (e) {
