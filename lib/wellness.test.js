...
describe("dailyLoadFromActivitiesAndLogs", () => {
  ...
  it("does not let a run activity consume an unrelated same-day strength log", () => {
    const out = dailyLoadFromActivitiesAndLogs({
      activities: [{ athlete_email: "a", activity_date: "2024-01-02", rtss: 80, activity_type: "Run", distance_km: "10", duration_seconds: 3000 }],
      logs: { s1: { id: "s1", athlete_email: "a", analysis: { actual_date: "2024-01-02", duration_min: 60, wellness: { rpe: 8 } } } },
      profile: {},
    });
    // Run keeps its 80; the distance-less strength log contributes its own
    // sRPE load (8 × 60 × 0.20 = 96) instead of vanishing.
    expect(out).toContainEqual({ date: "2024-01-02", rtss: 80 });
    expect(out).toContainEqual({ date: "2024-01-02", rtss: 96 });
  });
