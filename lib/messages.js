...
// True when an item (session log or activity) has coach content the athlete
// hasn't seen yet. Uses athlete_read_at as the watermark, so it covers BOTH
// the legacy coach_reply column and newer messages[] threads, and re-flags
// when the coach adds another message after a previous read. Pass the raw row.
export function athleteHasUnread(item) {
  if (!item) return false;
  const readAt = item.athlete_read_at || "";
  return getThread(item).some(m => m.author === "coach" && (m.created_at || "") > readAt);
}
