...
import {
  ...
  defaultRpeTarget, autoClassifyRunType, isLogReal, effectiveCompliance, formatStep,
} from "./load.js";

describe("formatStep interval duration units", () => {
  it("renders reps over 120s in minutes, short recoveries in seconds", () => {
    const s = formatStep({
      kind: "interval", reps: 5,
      work: { duration_s: 360, pace: "4:30 - 4:40" },
      recovery: { duration_s: 60, style: "float" },
    });
    expect(s).toBe("5 × 6 min @ 4:30 - 4:40 w/ 60 s float");
  });
  it("uses m:ss min for non-whole-minute reps", () => {
    const s = formatStep({ kind: "interval", reps: 3, work: { duration_s: 150 }, recovery: {} });
    expect(s).toContain("2:30 min");
  });
});
