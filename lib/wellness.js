...
// True when a same-day (email, date) activity/log pair plausibly describes
// the SAME session. Matching by date alone let a run activity "consume" an
// unrelated strength log on double-session days — dropping the strength load
// from ATL and polluting sRPE calibration with a mismatched RPE.
function plausiblePair(activity, log) {
  const akm = parseFloat(activity?.distance_km);
  const lkm = parseFloat(log?.analysis?.distance_km);
  if (akm && lkm) return Math.abs(akm - lkm) / Math.max(akm, lkm) < 0.25;
  if (akm && !lkm) return false; // distance activity vs distance-less log (e.g. strength) — different sessions
  const adur = activity?.duration_seconds ? activity.duration_seconds / 60 : null;
  const ldur = log?.analysis?.duration_min || null;
  if (adur && ldur) return Math.abs(adur - ldur) / Math.max(adur, ldur) < 0.3;
  return true; // not enough data to discriminate — keep legacy date pairing
}

function calibrateSrpeFactor({ activities, logs, profile }) {
...
function sessionLoad({ activity, log, profile, srpeFactor }) {
...
export function dailyLoadFromActivitiesAndLogs({ activities, logs, profile }) {
  ...
  for (const log of Object.values(logs || {})) {
    ...
    // Array per key: two same-day logs used to collide (last one silently won).
    if (!logsByDateEmail.has(key)) logsByDateEmail.set(key, []);
    logsByDateEmail.get(key).push(log);
  }
  ...
  for (const a of activities || []) {
    ...
    // First unconsumed log that plausibly describes THIS activity — a run
    // must not consume a same-day strength log (its load would vanish).
    const matchedLog = (logsByDateEmail.get(key) || [])
      .find(l => !consumedLogIds.has(l.id) && plausiblePair(a, l)) || null;
...
function dailyHooperSeries(logs) {
...
function hooperBaseline(seriesMap, asOfDate, opts = {}) {
...
// ─── 2b. HRV / autonomic readiness ──────────────────────────────────────────
// Morning HRV (rMSSD from HRV4Training / Apple Health / a chest strap) is the
// single strongest readiness marker. Higher = more recovered, so a drop BELOW
// the athlete's personal baseline signals stress — the opposite polarity to
// Hooper. Same robust baseline (rolling median + MAD) and quorum treatment.

const HRV_BASELINE_WINDOW_DAYS = 28;
const HRV_BASELINE_MIN_SAMPLES = 7;

// date → most-recently-logged HRV value (ms). Mirrors dailyHooperSeries.
function dailyHrvSeries(logs) {
  const byDate = new Map();
  const tsByDate = new Map();
  for (const log of Object.values(logs || {})) {
    const v = log?.analysis?.wellness?.hrv;
    const date = log?.analysis?.actual_date;
    if (v == null || !date) continue;
    const n = Number(v);
    if (!Number.isFinite(n) || n <= 0) continue;
    const ts = log.updated_at || log.created_at || "";
    const prevTs = tsByDate.get(date) || "";
    if (ts >= prevTs) { byDate.set(date, n); tsByDate.set(date, ts); }
  }
  return byDate;
}

// Today's HRV on the athlete's personal scale.
//   level: "fresh" | "normal" | "suppressed" | "alarm"
//   z:     z-score vs baseline (positive = above baseline = recovered)
// confidence "low" until both today's value and a 7+ sample baseline exist.
export function hrvToday({ logs, asOfDate }) {
  if (!asOfDate) return { confidence: "low", reason: "no date", level: null, z: null };
  const series = dailyHrvSeries(logs);
  const today = series.get(asOfDate);
  if (today == null) {
    return { confidence: "low", reason: "no HRV logged today", level: null, z: null };
  }
  const end = parseLocalDate(asOfDate);
  const start = new Date(end); start.setDate(end.getDate() - HRV_BASELINE_WINDOW_DAYS);
  const values = [];
  for (const [date, v] of series) {
    const d = parseLocalDate(date);
    if (d >= start && d < end) values.push(v);
  }
  if (values.length < HRV_BASELINE_MIN_SAMPLES) {
    return { confidence: "low", reason: `${values.length}/${HRV_BASELINE_MIN_SAMPLES} samples in window`, level: null, z: null, raw: today };
  }
  values.sort((a, b) => a - b);
  const median = values[Math.floor(values.length / 2)];
  const devs = values.map(v => Math.abs(v - median)).sort((a, b) => a - b);
  const mad = Math.max(devs[Math.floor(devs.length / 2)] || 0, 1); // floor 1ms, avoid div-by-zero
  const z = (today - median) / mad; // positive = above baseline = fresh
  let level;
  if (z >= 0.5) level = "fresh";
  else if (z > -0.5) level = "normal";
  else if (z > -1.5) level = "suppressed";
  else level = "alarm";
  return { confidence: "ok", level, z, raw: today, baseline: median, sampleCount: values.length };
}
...
//   }
export function readinessScore({ pmcTail, hooper, drift, hrv }) {
  const reasons = [];
  const inputsUsed = [];
  let cautionVotes = 0;
  let backOffVotes = 0;
  ...
  // HRV — autonomic suppression below the athlete's personal baseline.
  if (hrv && hrv.confidence === "ok") {
    inputsUsed.push("hrv");
    if (hrv.level === "suppressed") {
      cautionVotes++;
      reasons.push(`HRV below your baseline (z = ${hrv.z.toFixed(1)})`);
    } else if (hrv.level === "alarm") {
      backOffVotes++;
      reasons.push(`HRV well below baseline (z = ${hrv.z.toFixed(1)}) — autonomic fatigue`);
    }
  }
