...
import { RUN_TYPES } from "./helpers.js";
...
// Internal — exported solely for unit-test access if ever added.
function pairScore(session, activity) {
...
// Resolve linked activities for a set of sessions that may span multiple
// days (e.g. a whole plan history). Groups sessions and activities into
// per-date buckets — the pair scorer is date-blind, so cross-day
// candidates must be kept apart — then runs the matcher within each
// bucket. Counts ALL activity sources (strava-auto, strava, manual,
// session), which is the whole point: an athlete whose runs auto-sync
// never creates a source="session" row, so a session-only lookup would
// leave every prescribed session looking missed.
//
//   sessionsWithDate: [{ session, date }]   (date = effective YYYY-MM-DD)
//   activities:       full activity list (any source)
//   logs:             session_id → log map
// Returns Map<sessionId, activity|null>.
export function linkedActsBySession(sessionsWithDate, activities, logs) {
  const out = new Map();
  const actsByDate = new Map();
  for (const a of activities || []) {
    const d = a?.activity_date;
    if (!d) continue;
    if (!actsByDate.has(d)) actsByDate.set(d, []);
    actsByDate.get(d).push(a);
  }
  const sessionsByDate = new Map();
  for (const sd of sessionsWithDate || []) {
    if (!sd?.date || !sd.session) continue;
    if (!sessionsByDate.has(sd.date)) sessionsByDate.set(sd.date, []);
    sessionsByDate.get(sd.date).push(sd.session);
  }
  for (const [date, sessions] of sessionsByDate) {
    const m = matchActivitiesToSessions({ sessions, activities: actsByDate.get(date) || [], logs: logs || {} });
    for (const [sid, act] of m.bySessionId) out.set(sid, act);
  }
  return out;
}
